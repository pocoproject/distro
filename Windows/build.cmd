...
echo -------------------------------------------------------------------
Candle.exe -arch x86 -dVSYEAR=%VSYEAR% -dVERSION=%POCO_VERSION% -dPOCO=%POCO_BASE% -dPlatform=x86 -out "%VSYEAR%/Poco %POCO_VERSION% (x86).wixobj" -ext %VIXTOOLSET%\bin\WixUIExtension.dll Poco.wxs
...
echo -------------------------------------------------------------------
Candle.exe  -arch x64 -dVSYEAR=%VSYEAR% -dVERSION=%POCO_VERSION% -dPOCO=%POCO_BASE% -dPlatform=x64 -out "%VSYEAR%/Poco %POCO_VERSION% (x64).wixobj" -ext %VIXTOOLSET%\bin\WixUIExtension.dll Poco.wxs
